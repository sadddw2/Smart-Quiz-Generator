...
<div class="flex min-h-screen">
    ...
    <aside class="w-64 bg-white shadow-md p-6">
        ...
        <nav class="space-y-3">
            ...
            <a href="{{ route('kuis.index') }}" class="flex gap-3 items-center bg-purple-100 text-purple-700 px-4 py-3 rounded-xl font-semibold">📝 Data Kuis</a>
            ...
            <table class="w-full text-sm" id="kuisTable">
                ...
                <tbody>
                    ...
                    <tr class="border-b hover:bg-gray-50 kuis-row"
                        data-judul="{{ strtolower($kuis->judul_kuis) }}"
                        data-dosen="{{ strtolower($kuis->dosen) }}"
                        data-kesulitan="{{ strtolower($kuis->tingkat_kesulitan) }}">

                        <td class="p-4 text-center">{{ $index + 1 }}</td>

                        <td class="p-4">{{ $kuis->judul_kuis }}</td>

                        <td class="p-4">{{ $kuis->dosen }}</td>

                        <td class="p-4">{{ $kuis->jumlah_soal }}</td>

                        <td class="p-4">
                            <span class="px-3 py-1 rounded-full text-xs font-semibold
                                {{ strtolower($kuis->tingkat_kesulitan) == 'mudah'
                        ...
                        <td class="p-4">{{ $kuis->created_at->format('d M Y, H:i') }}</td>

                        <td class="p-4 text-center">
                            <div class="flex justify-center gap-3 items-center">

                                <a href="{{ route('kuis.generate', $kuis->id) }}"
                                ...
                                <form action="{{ route('kuis.delete', $kuis->id) }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE')
