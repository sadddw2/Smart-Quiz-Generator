...
<div class="flex min-h-screen">
    ...
    <aside class="w-64 bg-white shadow-md p-6">
        ...
        <nav class="space-y-3">
            ...
            <a href="{{ route('hasilkuis.index') }}" class="flex gap-3 items-center bg-purple-100 text-purple-700 px-4 py-3 rounded-xl font-semibold">📊 Data Bank Soal</a>
        </nav>
    </aside>
            ...
            <table class="w-full text-sm" id="soalTable">
                ...
                <tbody>
                    ...
                    <tr class="border-b hover:bg-gray-50 soal-row"
                        data-pertanyaan="{{ strtolower($soal->pertanyaan) }}"
                        data-jawaban="{{ strtoupper($soal->jawaban_benar) }}"
                        data-kesulitan="{{ strtolower($soal->kesulitan) }}">
                        ...
                        <td class="p-4">{{ Str::limit($soal->pilihan_a, 40) }}</td>
                        <td class="p-4">{{ Str::limit($soal->pilihan_b, 40) }}</td>
                        <td class="p-4">{{ Str::limit($soal->pilihan_c, 40) }}</td>
                        <td class="p-4">{{ Str::limit($soal->pilihan_d, 40) }}</td>
                        ...
                        <td class="p-4">
                            <span class="px-3 py-1 rounded-full text-xs font-semibold
                                {{ strtolower($soal->kesulitan) == 'mudah'
                        ...
                        <td class="p-4 text-center">
                            <div class="flex justify-center gap-3">
                                ...
                                <button type="button"
                                    onclick="lihatSoal(
                                        `{{ addslashes($soal->pertanyaan) }}`,
                                        `{{ addslashes($soal->pilihan_a) }}`,
                                        `{{ addslashes($soal->pilihan_b) }}`,
                                        `{{ addslashes($soal->pilihan_c) }}`,
                                        `{{ addslashes($soal->pilihan_d) }}`,
                                        `{{ $soal->jawaban_benar }}`,
                                        `{{ $soal->kesulitan }}`
                                    )"
