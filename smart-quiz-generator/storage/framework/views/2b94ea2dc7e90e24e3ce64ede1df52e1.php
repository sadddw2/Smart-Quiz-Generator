...
<div class="flex min-h-screen">
    ...
    <aside class="w-64 bg-white shadow-md p-6">
        ...
        <nav class="space-y-3">
            ...
            <a href="<?php echo e(route('dosen.materi')); ?>" class="block px-4 py-3 rounded-xl hover:bg-gray-100">📄 Materi</a>
            <a href="<?php echo e(route('dosen.bank-soal')); ?>" class="block px-4 py-3 rounded-xl hover:bg-gray-100">📚 Bank Soal</a>
            <a href="<?php echo e(route('dosen.mahasiswa')); ?>" class="block bg-purple-100 text-purple-700 px-4 py-3 rounded-xl font-semibold">👨‍🎓 Mahasiswa</a>
        </nav>
    </aside>
    ...
    <main class="flex-1 p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-5 mb-8">
            <div class="bg-white rounded-2xl shadow p-5">
                ...
                <h3 class="text-2xl font-bold"><?php echo e($hasilKuis->count()); ?></h3>
            </div>

            <div class="bg-white rounded-2xl shadow p-5">
                <div class="text-3xl mb-2">✅</div>
                <p class="text-sm text-gray-500">Lulus</p>
                <h3 class="text-2xl font-bold"><?php echo e($hasilKuis->where('status', 'lulus')->count()); ?></h3>
            </div>

            <div class="bg-white rounded-2xl shadow p-5">
                <div class="text-3xl mb-2">❌</div>
                <p class="text-sm text-gray-500">Tidak Lulus</p>
                <h3 class="text-2xl font-bold"><?php echo e($hasilKuis->where('status', 'tidak_lulus')->count()); ?></h3>
            </div>

            <div class="bg-white rounded-2xl shadow p-5">
                <div class="text-3xl mb-2">⭐</div>
                <p class="text-sm text-gray-500">Rata-rata Nilai</p>
                <h3 class="text-2xl font-bold"><?php echo e(round($hasilKuis->avg('nilai') ?? 0)); ?></h3>
            </div>
        </div>
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $hasilKuis; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $hasil): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="border-b hover:bg-gray-50 hasil-row"
                            data-search="<?php echo e(strtolower(($hasil->nama_mahasiswa ?? '') . ' ' . ($hasil->mata_kuliah ?? '') . ' ' . $hasil->status)); ?>"
                            data-status="<?php echo e(strtolower($hasil->status)); ?>">

                            <td class="p-4 text-center"><?php echo e($hasil->id); ?></td>
                            <td class="p-4"><?php echo e($hasil->nama_mahasiswa ?? '-'); ?></td>
                            <td class="p-4"><?php echo e($hasil->kuis_id); ?></td>
                            <td class="p-4"><?php echo e($hasil->mata_kuliah ?? '-'); ?></td>
                            <td class="p-4"><?php echo e($hasil->jumlah_benar); ?></td>
                            <td class="p-4"><?php echo e($hasil->jumlah_salah); ?></td>
                            <td class="p-4 font-bold"><?php echo e($hasil->nilai); ?></td>

                            <td class="p-4">
                                <span class="px-3 py-1 rounded-full text-xs font-semibold
                                    <?php echo e($hasil->status == 'lulus'
                                        ? 'bg-green-100 text-green-700'
                                        : 'bg-red-100 text-red-700'); ?>">
                                    <?php echo e(ucfirst(str_replace('_', ' ', $hasil->status))); ?>

                                </span>
                            </td>

                            <td class="p-4"><?php echo e($hasil->created_at); ?></td>
                            <td class="p-4"><?php echo e($hasil->updated_at); ?></td>
                            ...
                            <td class="p-4 text-center">
                                <div class="flex justify-center gap-2">
                                    <button type="button"
                                        onclick="lihatHasil(
                                            '<?php echo e($hasil->id); ?>',
                                            '<?php echo e(addslashes($hasil->nama_mahasiswa ?? '-')); ?>',
                                            '<?php echo e($hasil->kuis_id); ?>',
                                            '<?php echo e(addslashes($hasil->mata_kuliah ?? '-')); ?>',
                                    ...
                                    <form action="<?php echo e(route('hasilkuis.delete', $hasil->id)); ?>" method="POST">
                                        <?php echo csrf_field(); ?>
...
</html><?php /**PATH C:\Users\ACER\smart-quiz-generator\resources\views/dosen/mahasiswa-dosen.blade.php ENDPATH**/ ?>